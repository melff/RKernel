...
Classes and constructor functions for string-related widgets (text areas
etc.)
...
- `handler`:
  ...
  A function that is called when content is submitted by the user
  hitting 'enter'.
...
- `ensure_option`:
  ...
  A boolean (logical) value, whether at least one option has to be
  activated
